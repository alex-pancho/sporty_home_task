...
class TestBetPlacementHappyPath:
    ...
    def test_successful_single_bet_placement_with_receipt_verification(
        ...
        assert abs(potential_payout - expected_payout) < 0.01, \
        ...
        assert "vs" in receipt_match, "Receipt must show match details (teams)"
        
        receipt_stake = page.get_receipt_stake()
        assert receipt_stake == stake, f"Receipt stake {receipt_stake} should match entered {stake}"
